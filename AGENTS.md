...
- **Drawer:** `components/shell/CmsDrawerContent.tsx` renders the `mobile_drawer` tree recursively (`DrawerEntry`) — up to three levels, collapsible groups, per-item `mobile_icon` (Lucide names). The drawer header renders `branding` through shared `resolveBrandingPresentation` (`logo_size` / `logo_variant`, logo via `resolveAssetUrl` + `logo_alt`, text fallback when no logo).
