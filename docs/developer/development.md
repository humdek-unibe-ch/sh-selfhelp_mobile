...
Implications:
- Layout/typography/forms/interactive styles render their HeroUI presentation on web. The few capabilities with no web equivalent (biometrics, camera/media capture, secure hardware) stay native-only and a native-only renderer shows the `WebPreviewUnsupported` notice instead of faking a broken render.
