...
*/
/**
 * Earliest web boot side-effect: strip the one-time `previewSession` code from
 * the URL BEFORE expo-router boots.
 *
 ...
 * `previewSession` destabilises expo-router's web state<->URL round-trip: the
 * linking effect re-pushes the root route on every commit, Chromium throttles
 * the `history.pushState` flood ("Throttling navigation to prevent the browser
 * from hanging"), and the embedded Live Preview pane hangs on "Starting up…".
 *
